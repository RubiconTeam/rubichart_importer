...
static func convert(reader : FileAccess) -> RubiChart:
	...
	for i in amtOfCharts:
		...
		for j in target_switch_count:
			...
			var ts_name_length : int = sign_uint32(reader.get_32())
		...
		var sv_change_count : int = sign_uint32(reader.get_32())
		...
		var note_count : int = sign_uint32(reader.get_32())
		var notes : Array[NoteData] = []
		for j in note_count:
			var note : NoteData = NoteData.new()
			var serialized_type : int = reader.get_8()
			note.Time = reader.get_float()
			note.Lane = sign_uint32(reader.get_32())
			
			match serialized_type:
				1: # Typed tap note
					note.Type = note_types[sign_uint32(reader.get_32())]
				2: # Tap note with params
					var param_count : int = sign_uint32(reader.get_32())
					for k in param_count:
						var param_name_length : int = sign_uint32(reader.get_32())
						var param_name : StringName = reader.get_buffer(param_name_length).get_string_from_utf8()
						var param_value_length : int = sign_uint32(reader.get_32())
...
static func sign_uint32(unsigned : int):
